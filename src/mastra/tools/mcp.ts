...
// Validate required SMITHERY_API env var with error handling
const getSmitheryApi = (): string | null => {
  try {
    return z.string().nonempty().parse(process.env.SMITHERY_API);
  } catch (error) {
    logger.warn('SMITHERY_API environment variable not configured. Smithery-based MCP servers will be disabled.');
    return null;
  }
};

const smitheryApi = getSmitheryApi();
...
//    }
  ...
  async getToolsArray() {
    ...
    try {
      const stdioTools = await mcpStdio.getTools();
      tools.push(...Object.values(stdioTools));
    } catch (error) {
      logger.error('Failed to get Stdio MCP tools', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
...
export function getMCPAnalytics() {
  return {
    ...
    configuration: {
      smitheryApiConfigured: !!smitheryApi,
      totalServers: 5
    }
  };
}
